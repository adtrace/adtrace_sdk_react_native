...
static NSString * const ADTMethodGET = @"MethodGET";
static NSString * const ADTMethodPOST = @"MethodPOST";

@interface ADTRequestHandler()

@property (nonatomic, strong) ADTUrlStrategy *urlStrategy;
@property (nonatomic, copy) NSString *userAgent;
@property (nonatomic, assign) double requestTimeout;
@property (nonatomic, weak) id<ADTResponseCallback> responseCallback;
        ...
        sendingParameters:(NSDictionary *)sendingParameters
{
    ...
    ADTActivityKind activityKind = activityPackage.activityKind;

    ADTResponseData *responseData =
        [ADTResponseData buildResponseData:activityPackage];
...

{
    if (authorizationHeader != nil) {
        [ADTAdtraceFactory.logger debug:@"Authorization header content: %@", authorizationHeader];
...
- (void)retryWithResponseData:(ADTResponseData *)responseData
               methodTypeInfo:(NSString *)methodTypeInfo
{
    ADTActivityPackage *activityPackage = responseData.sdkPackage;
    NSDictionary *sendingParameters = responseData.sendingParameters;

    if (methodTypeInfo == ADTMethodGET) {
                       ...
                       responseData:(ADTResponseData *)responseData
{
    ...
    NSString *responseString = [[[NSString alloc]
                                 initWithData:data encoding:NSUTF8StringEncoding] adtTrim];
                       ...
                       activityKind:(ADTActivityKind)activityKind
{
    ...
    NSString *activityKindS = [ADTActivityKindUtil activityKindToString:activityKind];
...
- (void)saveJsonResponse:(NSData *)jsonData responseData:(ADTResponseData *)responseData {
    ...
    } else if ([ADTUtil isNull:jsonDict]) {
